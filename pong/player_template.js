...
export default class Player extends THREE.Mesh {
  ...
  doLowerBoundary(currentPosition, coveredDistance) {
    const newPosition = currentPosition - coveredDistance;
    // console.log(`lower ${this.centerLower} ${newPosition}`);
    return newPosition > this.centerLower ? newPosition : currentPosition;
  }

  doUpperBoundary(currentPosition, coveredDistance) {
    const newPosition = currentPosition + coveredDistance;
    // console.log(`upper ${this.centerUpper} ${newPosition}`);
    return newPosition < this.centerUpper ? newPosition : currentPosition;
  }

  initialize() {
    this.center = new THREE.Vector2(this.baseline, 0.0);
    if (this.side == "left") {
      // Player 1 racket: the center's x-coordinate must be negative
  ...
  update(deltaT) {
    ...
    let newPosition = this.center.y;

    if (this.keyStates.down) {
      newPosition = this.doLowerBoundary(this.center.y, coveredDistance);
    }
    if (this.keyStates.up) {
      newPosition = this.doUpperBoundary(this.center.y, coveredDistance);
    }
    this.center.y = newPosition;
    this.position.set(this.center.x, this.center.y);
  }
}
