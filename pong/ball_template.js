...
export default class Ball extends THREE.Mesh {
  ...
  initialize() {
    this.center = new THREE.Vector2(
      0.0,
      THREE.MathUtils.randFloatSpread(this.table.size.y - 4.0 * this.radius)
    );
  ...
  update(deltaT) {
    ...
    // player1: (${this.player1.center.x}, ${this.player1.center.y})
    // player2: (${this.player2.center.x}, ${this.player2.center.y})
    // `);
    ...
    if (
      ...
      this.center.x - this.radius <=
        this.player1.center.x + this.player1.halfSize.x &&
      this.center.y + this.radius <
        this.player1.center.y + this.player1.halfSize.y &&
      this.center.y - this.radius >
        this.player1.center.y - this.player1.halfSize.y
    ) {
    ...
    if (
      centerIncrement.x > 0 &&
      this.center.x + this.radius >=
        this.player2.center.x - this.player2.halfSize.x &&
      this.center.y + this.radius <
        this.player2.center.y + this.player2.halfSize.y &&
      this.center.y - this.radius >
        this.player2.center.y - this.player2.halfSize.y
    ) {
    ...
    if (
      centerIncrement.y > 0 &&
      this.center.y + this.radius >= this.table.halfSize.y
    ) {
      console.log("hit up");
      this.direction *= -1;
    } else if (
