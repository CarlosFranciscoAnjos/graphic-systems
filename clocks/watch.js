...
export default class Watch extends THREE.Group {
  constructor(
    ...
    let points = [];
    const numMarkers = 60;
    const increment = (Math.PI * 2) / numMarkers;

    function createMarkerVector(rad, mod, t) {
      const x = rad * mod * Math.cos(t);
      const y = rad * mod * Math.sin(t);
      return new THREE.Vector3(x, y, 0);
    }

    for (let i = 0; i < numMarkers; i++) {
      const t = increment * i;
      const mod = i % 5 == 0 ? 0.85 : 0.9;
      points.push(createMarkerVector(radius, 0.95, t));
      points.push(createMarkerVector(radius, mod, t));
    }
